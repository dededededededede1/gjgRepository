...
1.将静态页面放入nginx中的html文件夹中

2.避免两个服务器之间得访问跨域问题，ajax发送请求访问servlet需要加上全路径

`url: 'http://localhost:8080/mybtis-servlet/selectServlet', `
...
public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws IOException, ServletException {
    ...
    response.setHeader("Access-Control-ALLow-Origin","*");// 允许访阅域名
response.setHeader("Access-Control-Allow-Methods","POST,GET,OPTIONS");//允许访问的请求
response.setHeader("Access-Control-Max-Age","3600");// 缓存时间
response.setHeader("Access-Control-Allow-Headers","content-type , token");//自定义的请求
...
server代码块中
    ...
    charset utf-8;
        #设置反向代理，这样通过location后面的名字，前端ajax请求时，就能找到proxy_pass后面的东西
        location /mybtis-servlet  {
...
***如果出现在nginx中404的错误，可能是进程中启动的服务太多了，关掉重新启动nginx***
...
![Nginx](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e967b9714ed~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
***正向代理是知道了要请求的服务器，但是交给代理服务器去发这个请求。服务器不知道是哪个客户端发过来的请求。***

***反向代理是不知道要给哪个服务器去处理这个请求，交给代理服务器去找具体的服务器进行处理。客户端直接与代理服务器进行交互。***
...
正向代理和反向代理主要的原理区别可以参见下图：
    ...
    ![正向代理与反向代理](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96d99eecdc~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200307180412726](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e967ad35156~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200307180545816](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e967cd99d22~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)

### 3.2 相关文件夹

然后我们可以使用 `rpm -ql nginx` 来查看 Nginx 被安装到了什么地方，有哪些相关目录，其中位于 `/etc` 目录下的主要是配置文件，还有一些文件见下图：

![Xnip2020-03-07_21-46-11](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e969c75753f~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200426150644768](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96a168c66a~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200426153006505](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96a60a5e51~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200311153131642](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96a6d7033c~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200427220536208](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96b16214cc~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200428191153736](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96b99b44b7~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200428192028636](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96c8e70d22~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200427110415809](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96dc471f0b~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
其他更全的配置信息可以查看 [&lt;官网文档ngx_http_gzip_module&gt;](https://link.juejin.cn?target=http%3A%2F%2Fnginx.org%2Fen%2Fdocs%2Fhttp%2Fngx_http_gzip_module.html "http://nginx.org/en/docs/http/ngx_http_gzip_module.html")，配置前是这样的：

![image-20200427161022215](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96dd2b5019~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)

配置之后 response 的 header 里面多了一个 `Content-Encoding: gzip`，返回信息被压缩了：

![image-20200427164033577](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96def4db0f~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)

注意了，一般 gzip 的配置建议加上 `gzip_min_length 1k`，不加的话：

![image-20200427164408389](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96ee3279e8~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![image-20200427144824829](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e96f98708c7~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
...
![62haogU3DtwMRiZ](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/29/171c4e97062c5124~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.awebp)
